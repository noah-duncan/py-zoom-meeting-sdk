...
def run():
    ...
    def join_meeting():
        ...
        audioSettings = setting_service.GetAudioSettings()
    ...
    def start_raw_recording():
        nonlocal m_audioSource, audioHelper
        ...
        canstartrecording_result = recording_ctrl.CanStartRawRecording()

        if canstartrecording_result != zoom.SDKERR_SUCCESS:
            recording_ctrl.RequestLocalRecordingPrivilege()
            print("Cannot record yet")
            return

        startrawrecording_result = recording_ctrl.StartRawRecording()
        print("startrawrecording_result")
        print(startrawrecording_result)

        audioHelper = zoom.GetAudioRawdataHelper();
        if audioHelper is None:
            print("audioHelper is None")
            return "BAD"
        if m_audioSource is None:
            mixedAudio = False
            transcribe = False
            m_audioSource = zoom.ZoomSDKAudioRawDataDelegate(mixedAudio, transcribe)
            m_audioSource.setDir("out")
            m_audioSource.setFilename("test.pcm")
            print("set some shit")

        audioHelper_subscribe_result = audioHelper.subscribe(m_audioSource, False)
        print("audioHelper_subscribe_result")
        print(audioHelper_subscribe_result)
        print("z")
